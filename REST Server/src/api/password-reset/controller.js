...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Azaad Insurance account.<br>
        ...
        &mdash; Azaad Insurance Team
      `
      return sendMail({ toEmail: email, subject: 'Azaad Insurance - Password Reset', content })
    })
